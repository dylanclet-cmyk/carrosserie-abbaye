      ...
      {showPermissions && selectedSalarie && (
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#FFFFFF', borderRadius: 16, padding: '1.5rem', width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' as const }}>
            ...
            {/* Heures contrat */}
            <div style={{ marginBottom: 20, padding: '14px 16px', background: '#FFF8F3', borderRadius: 10, border: '1px solid #E8C8A0' }}>
              <label style={{ ...labelStyle, color: '#C8723A' }}>Heures contractuelles / semaine</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <input
                  type="number"
                  defaultValue={selectedSalarie.heures_contrat || 35}
                  min={1} max={48}
                  style={{ width: 80, padding: '8px 12px', borderRadius: 8, border: '1.5px solid #EDE5D8', fontSize: 16, fontWeight: 500, color: '#1A1A1A', background: '#FFFFFF', textAlign: 'center' as const }}
                  onChange={async (e) => {
                    const val = parseFloat(e.target.value)
                    if (!isNaN(val) && val > 0) {
                      await supabase.from('salaries').update({ heures_contrat: val }).eq('id', selectedSalarie.id)
                      setSalaries(salaries.map(s => s.id === selectedSalarie.id ? { ...s, heures_contrat: val } : s))
                      setSelectedSalarie((prev: any) => prev ? { ...prev, heures_contrat: val } : prev)
                    }
                  }}
                />
                <span style={{ fontSize: 14, color: '#888' }}>heures / semaine</span>
                <span style={{ fontSize: 12, color: '#C8723A', background: '#FFF0E6', padding: '3px 10px', borderRadius: 20 }}>
                  {selectedSalarie.heures_contrat || 35}h actuelles
                </span>
              </div>
              <div style={{ fontSize: 11, color: '#888', marginTop: 6 }}>Modifié en temps réel — utilisé pour le compteur d'heures du dashboard</div>
            </div>
