    ...
    <div style={{ minHeight: '100vh', background: '#f8f6f3', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ padding: '1.5rem 2rem', maxWidth: 960, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {dossiers.length === 0 ? (
            <div style={{ background: 'white', borderRadius: 12, padding: '2rem', textAlign: 'center', color: '#888', border: '1px solid #e8e2d9' }}>Aucun dossier</div>
          ) : dossiers.map(d => {
              ...
              <div key={d.id} style={{ background: 'white', borderRadius: 12, padding: '1rem 1.25rem', border: '1px solid #e8e2d9', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: '#888', marginTop: 2 }}>{d.clients?.prenom} {d.clients?.nom} - Entree le {new Date(d.date_entree).toLocaleDateString('fr-FR')}</div>
                  </div>
                </div>
